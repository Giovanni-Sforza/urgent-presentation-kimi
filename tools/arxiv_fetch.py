...
download-source  Download a paper's TeX source tarball and extract images.
...
python3 tools/arxiv_fetch.py download-source 2301.07041 --dir papers
"""
...
def _extract_images_from_source(src_dir: Path) -> list[str]:
    """Find image files inside extracted arXiv source directory."""
    images = []
    for path in src_dir.rglob("*"):
        if path.is_file() and path.suffix.lower() in _IMAGE_EXTENSIONS:
            images.append(str(path.relative_to(src_dir)))
    images.sort()
    return images


def download_source(arxiv_id: str, output_dir: str = "papers") -> dict:
    """Download a paper's TeX source tarball, extract it, and return image paths."""
    clean_id = _normalize_id(arxiv_id)
    safe_id = clean_id.replace("/", "_")

    dest_dir = Path(output_dir)
    dest_dir.mkdir(parents=True, exist_ok=True)
    src_dir = dest_dir / f"{safe_id}_src"
    tar_path = dest_dir / f"{safe_id}_src.tar.gz"

    if src_dir.exists() and any(src_dir.iterdir()):
        images = _extract_images_from_source(src_dir)
        return {
            "id": clean_id,
            "path": str(src_dir),
    ...
    src_url = f"https://arxiv.org/e-print/{clean_id}"
    req = urllib.request.Request(src_url, headers={"User-Agent": _USER_AGENT})
    ...
    for attempt in (1, 2):
        ...
        raise RuntimeError(f"Failed to download source {src_url} after retries")

    if len(data) < _MIN_TAR_BYTES:
        raise ValueError(
            f"Downloaded source is only {len(data)} bytes - likely an error page, not a tarball"
        )
    ...
    with tarfile.open(tar_path, "r:gz") as tf:
        tf.extractall(path=src_dir, filter="fully_trusted")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    source_parser = subparsers.add_parser(
        "download-source",
        help="Download a paper's TeX source tarball and extract images",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "download-source":
        result = download_source(args.id, output_dir=args.dir)
        if result.get("skipped"):
            print(json.dumps({**result, "message": "already exists, skipped"}, ensure_ascii=False))
        else:
            time.sleep(args.delay)
