...
This skill runs **after** Workflow 3 (`/skill:paper-writing`). It takes a compiled paper and generates a print-ready poster for conference poster sessions. The poster extracts key content from the paper — it does **not** dump the full paper text onto a poster.
...
- **VENUE = `NeurIPS`** — Target venue, determines color scheme. Supported: `NeurIPS`, `ICML`, `ICLR`, `AAAI`, `ACL`, `EMNLP`, `CVPR`, `ECCV`, `GENERIC`. Override via argument (e.g., `/skill:paper-poster "— venue: ICML"`).
...
- **REVIEWER_MODEL = `default`** — Model used via Agent subagent for poster review.
...
> 💡 Override: `/skill:paper-poster "paper/" — venue: CVPR, size: A1, orientation: portrait, columns: 3`
...
> This phase uses **the executor visual assessment** on rendered poster images to iteratively refine layout, readability, and visual hierarchy — similar to the `paper-illustration` skill's review loop.
...
**Step 2: the executor visual assessment**
...
### Phase 6: Agent subagent Review

Send the poster content plan + key LaTeX sections to the reviewer subagent for review.
...
- **Feishu notifications are optional.** If `~/.kimi/aris/feishu.json` exists, send notifications. Otherwise skip.
...
```
/skill:paper-poster "paper/" — venue: CVPR, size: A1, orientation: portrait, columns: 3
```
