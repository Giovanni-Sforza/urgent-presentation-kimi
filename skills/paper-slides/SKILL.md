...
This skill runs **after** Workflow 3 (`/skill:paper-writing`). It takes a compiled paper and generates a presentation slide deck for conference oral talks, spotlight presentations, or poster lightning talks.
...
- **REVIEWER_MODEL = `default`** — Model used via Agent subagent for slide review.
...
> 💡 Override: `/skill:paper-slides "paper/" — talk_type: oral, venue: ICML, minutes: 20, aspect: 4:3`
...
### Phase 5: Agent subagent Review

Send the slide outline + selected LaTeX frames to the reviewer subagent:
...
> ⚠️ If the Agent tool is not available (no OpenAI API key), skip external review and proceed to Phase 6. Note the skip in `SLIDES_STATE.json`.
...
- Files:
  ...
  ├── SLIDES_REVIEW.md      # the reviewer subagent review feedback
...
- **Feishu notifications are optional.** If `~/.kimi/aris/feishu.json` exists, send notifications. If absent, skip.

## Parameter Pass-Through

```
/skill:paper-slides "paper/" — talk_type: oral, venue: ICML, minutes: 20, aspect: 4:3, notes: false
```
