...
description: "Generate publication-quality AI illustrations for academic papers using Gemini image generation. Creates architecture diagrams, method illustrations with the executor-supervised iterative refinement loop. Use when user says \"生成图表\", \"画架构图\", \"AI绘图\", \"paper illustration\", \"generate diagram\", or needs visual figures for papers."
argument-hint: [description-or-method-file]
---

# Paper Illustration: Multi-Stage the executor-Supervised Figure Generation
...
│       ▼                                                                  │
│   ┌─────────────┐                                                        │
│   │   the executor    │ ◄─── Step 1: Parse request, create initial prompt     │
│   │  (Planner)  │                                                        │
│   └──────┬──────┘                                                        │
│          │                                                               │
│          ▼                                                               │
...
│          ▼                                                               │
│   ┌─────────────┐                                                        │
│   │   the executor    │ ◄─── Step 5: STRICT visual review + SCORE (1-10)      │
...
**Not for:** Statistical plots (use `/skill:paper-figure`), photo-realistic images
...
### Step 5: the executor STRICT Visual Review & Scoring (MANDATORY)
...
```markdown
## the executor's STRICT Review of Figure v{N}
...
11. **USE MULTI-STAGE WORKFLOW** — the executor → Gemini Layout → Gemini Style → Paperbanana → the executor Review
